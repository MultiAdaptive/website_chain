...
function updateCurrBlobNum(state: string, action: { type: string, blobNum: string }) {
    switch (action.type) {
        case 'update':
            return action.blobNum;
        case 'delete':
             return '';
        default:
            return state;
...
export function BlobProvider({ children }: { children: React.ReactNode }){
    ...
    const [searchVal, dispatchSearchVal] = useReducer<Reducer<{ key: string, value: any }, {type: string, searchVal: { key: string, value: any }}>>(updateSearchVal, { key: 'All Filters', value: '' })
    const [currBlobNum, dispatchCurrBlobNum] = useReducer<Reducer<string, { type: string, blobNum: string }>>(updateCurrBlobNum, '')
